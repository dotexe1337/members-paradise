...
namespace FivemPlayerlist
{
    public class FivemPlayerlist : BaseScript
    {
        ...
        public FivemPlayerlist()
        {
            ...
            TriggerEvent("fs:setPlayerRowConfig", GetPlayerServerId(Game.Player.Handle), "", -1, false);
        }
        ...
        private async Task LoadScale()
        {
            ...
            var titleIcon = "2";
            var titleLeftText = "Members Paradise";
                ...
                if (IsRowSupposedToShow(amount))
                {
                    ...
                    if (playerConfigs.ContainsKey(p.ServerId))
                    {
                        row = new PlayerRow()
                        {
                            ...
                            name = (IsAceAllowed("group.owner") ? "Owner" : IsAceAllowed("group.admin") ? "Admin" : IsAceAllowed("group.developer") ? "Developer" : IsAceAllowed("group.moderator") ? "Moderator" : "Player") +  p.Name.Replace("<", "").Replace(">", "").Replace("^", "").Replace("~", "").Trim(),
